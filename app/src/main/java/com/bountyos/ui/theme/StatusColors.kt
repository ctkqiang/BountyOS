...
fun statusColor(status: SubmissionStatus, isDark: Boolean): Color = when (status) {
    SubmissionStatus.OPEN -> if (isDark) CoolNeutral else LightNeutral
    ...
    SubmissionStatus.DUPLICATE -> if (isDark) MutedNeutral else LightNeutral
    SubmissionStatus.INFORMATIVE -> if (isDark) CoolNeutral else LightNeutral
    SubmissionStatus.UNKNOWN -> if (isDark) MutedNeutral else LightNeutral
}
